...
#    model = GNN_EA(params, n_atom_features=getatomfeaturelen(True))
    model = GNN_EA(getatomfeaturelen(True), n_heads=params['n_heads'],
                   n_gat_layers = params['n_gat_layers'], dim_gat_feat = params['n_gat_layers'], 
                   d_FC_layer = params['dim_fcl_feat'], n_fcl = params['n_fcl'], 
    ...
    if len(list_posfile) == 0 and len(list_negfile) ==0:
        ...
        exit()
    print(f'{len(list_posfile)}+{len(list_negfile)} files being used for training.', flush=True)
    ...
    print("Epochs and mini-batches: ", n_epochs, int(len(train_dataloader)), flush=True)
    for k in range(n_epochs):
        ...
        print(f'Epoch {k} laptime:', time.time() - starttime, flush=True)
        if k % 5 == 0:
